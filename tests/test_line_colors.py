import json
from pathlib import Path

import fetch

DATA_DIR = Path(__file__).resolve().parent.parent / "src" / "data"
LINE_COLORS_PATH = DATA_DIR / "line-colors.json"
LINE_NAMES_PATH = DATA_DIR / "line-names.json"


def _load(path):
    return json.loads(path.read_text(encoding="utf-8"))


def test_covers_exactly_the_lines_fetch_py_produces():
    """A line added to/removed from fetch.py's LINES dict without a matching update
    here would silently render trains with no colour (or a stale one) on the map."""
    colors = _load(LINE_COLORS_PATH)
    assert set(colors) == set(fetch.LINES)


def test_every_value_is_a_hex_colour():
    colors = _load(LINE_COLORS_PATH)
    for line_id, value in colors.items():
        assert isinstance(value, str) and len(value) == 7 and value[0] == "#", (
            f"{line_id!r} has a non-hex value: {value!r}"
        )
        int(value[1:], 16)  # raises ValueError if not valid hex digits


def test_line_names_covers_exactly_the_lines_fetch_py_produces():
    names = _load(LINE_NAMES_PATH)
    assert set(names) == set(fetch.LINES)


def test_line_names_match_fetch_py_display_names():
    """These are hand-copied from fetch.py's LINES dict for the frontend legend;
    this test is the tripwire if the two are ever edited independently."""
    names = _load(LINE_NAMES_PATH)
    assert names == fetch.LINES
